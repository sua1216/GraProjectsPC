 ...
 * three坐标与经纬度互转
 * @author bird & zhh
 * @class XBLngLatXYConvert - Convert
 * @version 1.0.1
 */
export default class XBLngLatXYConvert{
    ...
    /**
     * 经纬度转three坐标
     *    中心点center_latlng处坐标为0，地面向东为x轴，向北为z负轴，向天空为y正轴
    ...
    getXY(convert_lnglat, center_lnglat){
        ...
        {
            throw new Error("XBLngLatXYConvert getXY error");
        }
        ...
        if (ratio === 0) {
            ...
            return {
                x: (world_pos.x - center_world_pos.x) / ratio ,
                y: (world_pos.y - center_world_pos.y) / ratio
            };
        }
    ...
    getLngLat(xy, center_lnglat){
        if (
            ...
            xy.y === undefined)
        {
            throw new Error("XBLngLatXYConvert getLngLat error")
        }
        let center_world_pos = this._projectToWorld(center_lnglat);
        let ratio = this._projectedUnitsPerMeter(center_lnglat[1]);
        let world_pos ={};
